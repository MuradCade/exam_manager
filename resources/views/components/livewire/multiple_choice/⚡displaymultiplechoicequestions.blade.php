...
new class extends Component {
        ...
        $this->questionoptions = collect($optionRow->option_text ?? [])
            ->map(function ($text, $index) use ($optionRow) {
                return [
                    ...
                    'is_correct' => in_array($index, $optionRow->correct_option ?? []),
                ];
            })
            ->toArray();
    }
    ...
    public function update()
    {
        ...
        // validate if the user choice more then two answer as correct
        if (count($correctIndexes) !== 2) {
            $this->addError('questionoptions.is_correct_option', 'This is a multiple-choice question. Please mark exactly two correct answer.');
            return;
        }
        $this->resetErrorBag();

        // update the question
        DB::transaction(function() use($correctIndexes){
                                            ...
                                            @foreach ($optiondata->option_text as $index => $text)
                                                ...
                                                @else
                                                    <li style='color:black;'>
                                                        @if (in_array($index, $optiondata->correct_option ?? []))
                                                            <span
