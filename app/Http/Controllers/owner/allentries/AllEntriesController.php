...
class AllEntriesController
{
    public function index($exam_id)
    {
        ...
        // 1. Optimization: Use findOrFail to avoid manual if check, or at least select only needed columns
        $examform = examform::where('id', $examId)
            ->where('user_id', Auth::id())
            ->firstOrFail();

        // 2. Fetch questions and get total marks in one go
        // This avoids querying the questions table twice.
        $questions = examquestions::where('exam_id', $examId)
            ->with('options')
            ->get()
            ->keyBy('id');
        ...
        // 5. Scoring Logic (Optimized loop)
        $examType = $examform->exam_type; // Store in variable to avoid repeated property access

        $participantsdata->getCollection()->transform(function ($participant) use ($allAnswers, $questions, $examType) {
            $totalScore = 0;
            $answers = $allAnswers->get($participant->id) ?? collect();
            ...
            foreach ($answers as $answer) {
                ...
                // Cache decoded selected options to avoid decoding inside the options loop
                $selected = is_array($answer->selected_option)
                    ? $answer->selected_option
                    : json_decode($answer->selected_option, true) ?? [];
                ...
                foreach ($question->options as $option) {
                    if ($examType === 'single_choice') {
                        ...
                        // Optimization: array_intersect is faster than manual loops
